...
import { Gavel, FileText, PenTool, CheckCircle2, Columns, Eye, EyeOff, Plus, Minus, Trash2, Box, Package, Archive, Scale, Briefcase, ShoppingCart, ChevronDown, Zap, AlertTriangle, ShieldAlert, Info, MessageSquare } from 'lucide-react';
...
export const LicitacaoForm: React.FC<LicitacaoFormProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* PRIORITY BLOCK - ONLY VISIBLE IN STAGE 0 (Início) */}
      {viewingIndex === 0 && (
        <div className="space-y-4 border-t border-slate-200 pt-6">
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
            <ShoppingCart className="w-4 h-4 text-emerald-600" /> Prioridade
          </h3>
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
            <div className="pt-2">
              <label className={labelClass}>Prioridade da Licitação</label>
                ...
                {PRIORITY_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-[11px] font-black uppercase tracking-widest transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 ${colors[opt.color as keyof typeof colors]}`}
                    >
            ...
            {(content.priority === 'Alta' || content.priority === 'Urgência') && (
              <div className="pt-4 animate-slide-up">
                ...
                <div className="relative">
                  <textarea
                    ...
                    disabled={isActuallyReadOnly}
                    className={`${inputClass} min-h-[100px] resize-none leading-relaxed p-4 border-rose-100 bg-rose-50/20 disabled:bg-slate-100 disabled:border-slate-200`}
