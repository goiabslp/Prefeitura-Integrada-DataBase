...
export const FleetManagementScreen: React.FC<FleetManagementScreenProps> = ({
  ...
  const oilStats = useMemo(() => {
    ...
    // Timing Belt Stats
    const tbInDay = vehicles.filter(v => v.timingBeltNextChange && v.currentKm && (v.timingBeltNextChange - v.currentKm > 1000)).length;
    const tbNear = vehicles.filter(v => v.timingBeltNextChange && v.currentKm && (v.timingBeltNextChange - v.currentKm <= 1000 && v.timingBeltNextChange - v.currentKm > 0)).length;
    const tbExpired = vehicles.filter(v => v.timingBeltNextChange && v.currentKm && (v.timingBeltNextChange - v.currentKm <= 0)).length;

    return { inDay, near, expired, tbInDay, tbNear, tbExpired };
  }, [vehicles]);
            ...
            <div className="flex items-center gap-2 animate-fade-in pl-4 border-l border-slate-200 ml-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 mr-1 flex items-center gap-1">
                  <Activity className="w-3 h-3" /> Correia:
                </span>
                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-xl border border-emerald-200" title="Correia Dentada em Dia">
                  <span className="text-[10px] font-black">{oilStats.tbInDay}</span>
                </div>
                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-100 text-amber-700 rounded-xl border border-amber-200" title="Correia Dentada Próxima da Troca">
                  <span className="text-[10px] font-black">{oilStats.tbNear}</span>
                </div>
