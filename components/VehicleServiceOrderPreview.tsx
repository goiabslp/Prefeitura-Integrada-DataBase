...
export const VehicleServiceOrderPreview: React.FC<VehicleServiceOrderPreviewProps> = ({
    ...
    return createPortal(
        <div className={`fixed inset-0 z-[9999] flex items-start justify-center p-4 md:p-8 ${isGenerating ? 'bg-white' : 'bg-slate-900/95 backdrop-blur-md'} animate-fade-in overflow-y-auto modal-backdrop-os`}>

            {/* Botões FIXOS nas laterais (Desktop) */}
            <div className={`fixed left-6 top-1/2 -translate-y-1/2 z-[100] hidden xl:block ${isGenerating ? 'hidden' : ''}`}>
                <button
                    onClick={handleDownloadPdf}
                    disabled={isGenerating}
                    className="p-5 bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase tracking-[0.2em] text-[10px] rounded-3xl transition-all shadow-[0_20px_50px_rgba(79,70,229,0.3)] active:scale-95 flex flex-col items-center gap-3 border border-indigo-400 group disabled:opacity-50"
                >
                    <Printer className="w-6 h-6 group-hover:scale-110 transition-transform" />
                    <span className="[writing-mode:vertical-lr] rotate-180">{isGenerating ? 'Gerando...' : 'Gerar PDF / Imprimir'}</span>
                </button>
            </div>

            <div className={`fixed right-6 top-1/2 -translate-y-1/2 z-[100] hidden xl:block ${isGenerating ? 'hidden' : ''}`}>
                <button
                    onClick={onClose}
                    className="p-5 bg-white hover:bg-rose-50 text-slate-400 hover:text-rose-600 font-black uppercase tracking-[0.2em] text-[10px] rounded-3xl transition-all shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-200 active:scale-95 flex flex-col items-center gap-3 group"
                >
                    <X className="w-6 h-6 group-hover:rotate-90 transition-transform" />
                    <span className="[writing-mode:vertical-lr] rotate-180">Fechar Visualização</span>
                </button>
            </div>

            <div className={`relative w-full max-w-4xl min-h-full flex flex-col items-center py-12 ${isGenerating ? 'py-0' : ''}`}>
                {/* Botões Mobile/Tablet (mantém em cima) */}
                <div className={`flex xl:hidden w-full max-w-[800px] gap-3 mb-8 px-4 ${isGenerating ? 'hidden' : ''}`}>
                ...
                {/* Document Scale Wrapper to fit screen if needed */}
                <div className={`${isGenerating ? 'bg-white p-0' : 'bg-slate-200/50 p-8 rounded-[2rem] shadow-2xl'} overflow-auto custom-scrollbar max-h-full w-full flex justify-center`}>
